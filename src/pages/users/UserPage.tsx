...
const UserPage = () => {
    ...
    const createNewUser = () => {
        ...
        if(newUser.email.trim() === ''){
            ...
            setError(newError)   
        }else if(newUser.email.trim() !== '' && (!newUser.email.trim().includes('@') || !newUser.email.trim().includes('.'))){
            newError = {
                ...newError,
        ...
        if(newUser.password.trim() === ''){
            ...
            setError(newError)   
        }else if(newUser.password.trim() !== '' && newUser.password.trim().length < 8){
            newError = {
                ...newError,
    ...
    const editUser = () => {
        ...
        if(userSelected.email.trim() === ''){
            ...
            setError(newError)   
        }else if(userSelected.email.trim() !== '' && (!userSelected.email.trim().includes('@') || !userSelected.email.trim().includes('.'))){
            newError = {
                ...newError,
                email: ERROR_MESSAGES.INCORRECT_FIELD
            }
        ...
        if(userSelected.password.trim() === ''){
            ...
            setError(newError)   
        }else if(userSelected.password.trim() !== '' && userSelected.password.trim().length < 8){
            newError = {
                ...newError,
                password: ERROR_MESSAGES.INCORRECT_FIELD
            }
        ...
        if(newError.name === '' && newError.email === '' && newError.password === ''){
            ...
            const sortedUsers = [...usersFiltered, userSelected].sort((a: User, b: User) => a.id - b.id)
        ...
        <>
            <div className="overflow-x-auto">
                ...
                <Table theme={customTheme}>
                    <Table.Head>
                        ...
                        <Table.HeadCell className="text-center">Correo electrónico</Table.HeadCell>
                    ...
                    <Table.Body className="divide-y">
                        {
                            (users.length > 0) ? (users.map((user, i) => { 
                                ...
                                <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                    <Table.Cell className="text-center bg-gray-50" colSpan={4}>No hay usuarios disponibles</Table.Cell>
                                </Table.Row>
                            )
                        }
                    </Table.Body>
            ...
            <Modal show={openModalAdd} size="md" onClose={closeModalAdd} popup>
                <Modal.Header className="px-2 mb-5 flex items-center">
                    <h3 className="ml-5 text-xl font-medium text-gray-900 dark:text-white">Agregar nuevo usuario</h3>
                </Modal.Header>
                <Modal.Body>
                    <div className="space-y-6">
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewUser({ ...newUser, [e.target.name]: e.target.value })}
                                color={(newUser.name === '' && error.name === '') ? 'warning' : (error.name !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.name !== '' && (
                                        <>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewUser({ ...newUser, [e.target.name]: e.target.value })}
                                color={(newUser.email === '' && error.email === '') ? 'warning' : (error.email !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.email !== '' && (
                                        <>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewUser({ ...newUser, [e.target.name]: e.target.value })}
                                color={(newUser.password === '' && error.password === '') ? 'warning' : (error.password !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.password !== '' && (
                                        <>
                        ...
                        <div className="w-full">
                            <Button className="px-4" onClick={createNewUser} color="success">Agregar</Button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
            ...
            <Modal show={openModalEdit} size="md" onClose={closeModalEdit} popup>
                <Modal.Header className="px-2 mb-5 flex items-center">
                    <h3 className="ml-5 text-xl font-medium text-gray-900 dark:text-white">Editar impuesto usuario { userSelected && userSelected.name }</h3>
                </Modal.Header>
                <Modal.Body>
                    <div className="space-y-6">
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => userSelected && setUserSelected({...userSelected, [e.target.name]: e.target.value})}
                                color={(userSelected && userSelected.name === '' && error.name === '') ? 'warning' : (error.name !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.name !== '' && (
                                        <>
                                            <span className="font-medium">{error.name}</span>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => userSelected && setUserSelected({...userSelected, [e.target.name]: e.target.value})}
                                color={(userSelected && userSelected.email === '' && error.email === '') ? 'warning' : (error.email !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.email !== '' && (
                                        <>
                                            <span className="font-medium">{error.email}</span>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => userSelected && setUserSelected({...userSelected, [e.target.name]: e.target.value})}
                                color={(userSelected && userSelected.password === '' && error.password === '') ? 'warning' : (error.password !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.password !== '' && (
                                        <>
                                            <span className="font-medium">{error.password}</span>
                        ...
                        <div className="w-full">
                            <Button className="px-4" color="warning" onClick={editUser}>Editar</Button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
