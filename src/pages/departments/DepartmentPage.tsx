import { Alert, Breadcrumb, Button, CustomFlowbiteTheme, Label, Modal, Select, Table, TextInput } from "flowbite-react"
...
import { ROUTE_DEPARTMENTS_URL, ROUTE_HOME_URL } from "../../constants/routes/routes";
...
const DepartmentPage = () => {
    ...
    const [newDepartment, setNewDepartment] = useState<NewDepartment>(initialNewDepartment)
    ...
    const editDepartment = () => {
        ...
        if(departmentSelected.code === 0){
            ...
            setError(newError)
        }else if(Number.parseInt(departmentSelected.code.toString()) === 0 || Number.parseInt(departmentSelected.code.toString()).toString().length !== 6){
            newError = {
                ...newError,
        ...
        if(departmentSelected.areaName.trim() === ''){
            ...
            setError(newError)
        }else if(!areas.some(area => area.name.trim() === departmentSelected.areaName.trim())){
            newError = {
                ...newError,
                areaName: ERROR_MESSAGES.INCORRECT_FIELD
            }
        ...
        if(newError.code === '' && newError.name === '' && newError.areaName === ''){
            // todo llamada a la api para modificar los datos
            const departmentsFiltered = departments.filter(department => department.code !== departmentSelected.code)
            const sortedDepartments = [...departmentsFiltered, departmentSelected].sort((a: Department, b: Department) => a.code - b.code)
        ...
        <>
            <div className="overflow-x-auto">
                <div className="flex mt-2 mb-10 p-3 rounded-md bg-gray-50 dark:bg-gray-800 items-center justify-between">
                    ...
                    <Button color="success" size='xs' onClick={() => setOpenModalAdd(true)}>Agregar departamento</Button>
                </div>
                                ...
                                return <Table.Row key={i} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    <Table.Cell>
                                        <div className="flex gap-4">
                                            ...
                                            <button type="button" onClick={() => handleDelete(department)} className="font-medium text-red-600 dark:text-cyan-500">
                                                Eliminar
                                            </button>
                                        </div>
                                    </Table.Cell>
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewDepartment({ ...newDepartment, [e.target.name]: e.target.value })}
                                color={(newDepartment.name === '' && error.name === '') ? 'warning' : (error.name !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.name !== '' && (
                                        <>
                        ...
                        </div>
                        {
                            areas.length !== 0 && (
                                <div>
                                    ...
                                    <Select id="areaName" name="areaName" required onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setNewDepartment({ ...newDepartment, [e.target.name]: e.target.value })}>
                                        {
                                            areas.map((area, i) => {return <option key={i} value={area.name} defaultChecked={newDepartment.areaName === area.name}>{area.name}</option>})
                                        }
                                    </Select>
                                </div>
                            )
                        ...
                        <div className="w-full">
                            <Button className="px-4" onClick={createNewDepartment} color="success" disabled={areas.length === 0}>Agregar</Button>
                            {
                                areas.length === 0 && (<Alert className="mt-4" color='warning' withBorderAccent>
                                    <span><span className="font-medium">Alerta</span> No hay áreas disponibles para asignarle al departamento</span>
                                </Alert>)
                            }
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => departmentSelected && setDepartmentSelected({...departmentSelected, [e.target.name]: e.target.value})}
                                color={(departmentSelected && departmentSelected.code === 0 && error.code === '') ? 'warning' : (error.code !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.code !== '' && (
                                        <>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => departmentSelected && setDepartmentSelected({...departmentSelected, [e.target.name]: e.target.value})}
                                color={(departmentSelected && departmentSelected.name === '' && error.name === '') ? 'warning' : (error.name !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.name !== '' && (
                                        <>
                                            <span className="font-medium">{error.name}</span>
                        ...
                        </div>
                        {
                            areas.length !== 0 && (
                                <div>
                                    ...
                                    <Select id="areaName" name="areaName" required onChange={(e: React.ChangeEvent<HTMLSelectElement>) => departmentSelected && setDepartmentSelected({ ...departmentSelected, [e.target.name]: e.target.value })}>
                                        {
                                            areas.map((area, i) => {return <option key={i} value={area.name} defaultChecked={departmentSelected && departmentSelected.areaName === area.name}>{area.name}</option>})
                                        }
                                    </Select>
                                </div>
                            )
                        ...
                        <div className="w-full">
                            <Button className="px-4" color="warning" onClick={editDepartment} disabled={areas.length === 0}>Editar</Button>
                            {
                                areas.length === 0 && (<Alert className="mt-4" color='warning' withBorderAccent>
                                    <span><span className="font-medium">Alerta</span> No hay áreas disponibles para asignarle al departamento</span>
                                </Alert>)
            ...
            <Modal show={openModalDelete} size="md" onClose={closeModalDelete} popup>
                ...
                <div className="text-center">
                    ...
                    <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        ¿Estás seguro de eliminar al departamento {departmentSelected && departmentSelected.name}?
                    </h3>
