...
const AreaPage = () => {
    ...
    const editArea = () => {
        ...
        if(newError.name === ''){
            ...
            const areasFiltered = areas.filter(area => area.code !== areaSelected.code)
            const sortedAreas = [...areasFiltered, areaSelected].sort((a: Area, b: Area) => a.code - b.code)
        ...
        <>
            <div className="overflow-x-auto">
                <div className="flex mt-2 mb-10 p-3 rounded-md bg-gray-50 dark:bg-gray-800 items-center justify-between">
                    ...
                    <Button color="success" size='xs' onClick={() => setOpenModalAdd(true)}>Agregar área</Button>
                </div>
                    ...
                    <Table.Body className="divide-y">
                        {
                            (areas.length > 0) ? (areas.map((area, i) => { 
                                return <Table.Row key={i} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    <Table.Cell>
                                        <div className="flex justify-center gap-4">
                                            <button type="button" onClick={() => handleEdit(area)} className="font-medium text-yellow-300 dark:text-yellow-400">
                                                Editar
                                            </button>
                                            <button type="button" onClick={() => handleDelete(area)} className="font-medium text-red-600 dark:text-cyan-500">
                                                Eliminar
                                            </button>
                                        </div>
                                    </Table.Cell>
                                ...
                                <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                    <Table.Cell className="text-center bg-gray-50" colSpan={3}>No hay áreas disponibles</Table.Cell>
                                </Table.Row>
                            )
                        }
                    </Table.Body>
            ...
            <Modal show={openModalAdd} size="md" onClose={closeModalAdd} popup>
                <Modal.Header className="px-2 mb-5 flex items-center">
                    <h3 className="ml-5 text-xl font-medium text-gray-900 dark:text-white">Agregar nueva área</h3>
                </Modal.Header>
                <Modal.Body>
                    <div className="space-y-6">
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewArea({ ...newArea, [e.target.name]: e.target.value })}
                                color={(newArea.name === '' && error.name === '') ? 'warning' : (error.name !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.name !== '' && (
                                        <>
                        ...
                        <div className="w-full">
                            <Button className="px-4" onClick={createNewArea} color="success">Agregar</Button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
            ...
            <Modal show={openModalEdit} size="md" onClose={closeModalEdit} popup>
                <Modal.Header className="px-2 mb-5 flex items-center">
                    <h3 className="ml-5 text-xl font-medium text-gray-900 dark:text-white">Editar área { areaSelected && areaSelected.name }</h3>
                </Modal.Header>
                <Modal.Body>
                    <div className="space-y-6">
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => areaSelected && setAreaSelected({...areaSelected, [e.target.name]: e.target.value})}
                                color={(areaSelected && areaSelected.name === '' && error.name === '') ? 'warning' : (error.name !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.name !== '' && (
                                        <>
                                            <span className="font-medium">{error.name}</span>
                        ...
                        <div className="w-full">
                            <Button className="px-4" color="warning" onClick={editArea}>Editar</Button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
            ...
            <Modal show={openModalDelete} size="md" onClose={closeModalDelete} popup>
                ...
                <div className="text-center">
                    ...
                    <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        ¿Estás seguro de eliminar el área {areaSelected && areaSelected.name}?
                    </h3>
                    <div className="flex justify-center gap-4">
