...
const TaxPage = () => {
    ...
    useEffect(() => {
        ...
        axios.get(GET_LIST_TAXS).then(({data}) => {
            let taxs: Tax[] = data
            setSegTaxs(taxs.filter(tax => tax.type === 'seguridad social').sort((a, b) => a.minValue - b.minValue))
            setIngTaxs(taxs.filter(tax => tax.type === 'ingresos personales').sort((a, b) => a.minValue - b.minValue))
        })
    }, [])
    ...
    const createNewTax = () => {
        ...
        if(newTax.type.trim() === ''){
            ...
            setError(newError)
        }else if(newTax.type.trim() !== 'ingresos personales' && newTax.type.trim() !== 'seguridad social'){
            newError = {
                ...newError,
        ...
        if(newError.type === '' && newError.minValue === '' && newError.maxValue === '' && newError.percentage === ''){
            if(newTax.type === 'ingresos personales'){
                // todo llamada a la api para obtener un id disponible
                const newId = 100
                const sorted = [...ingTaxs, { id: newId, ...newTax }].sort((a: Tax, b: Tax) => a.minValue - b.minValue)
                setIngTaxs(sorted)
            }
            ...
            if(newTax.type === 'seguridad social'){
                // todo llamada a la api para obtener un id disponible
                const newId = 100
                const sorted = [...segTaxs, { id: newId, ...newTax }].sort((a: Tax, b: Tax) => a.minValue - b.minValue)
                setSegTaxs(sorted)
            }
    ...
    const editTax = () => {
        ...
        if(taxSelected.type.trim() === ''){
            ...
            setError(newError)
        }else if(taxSelected.type.trim() !== 'ingresos personales' && taxSelected.type.trim() !== 'seguridad social'){
            newError = {
                ...newError,
                type: ERROR_MESSAGES.INCORRECT_FIELD
            }
        ...
        if(newError.type === '' && newError.minValue === '' && newError.maxValue === '' && newError.percentage === ''){
            ...
            if(taxSelected.type === 'ingresos personales'){
                const taxsFiltered = ingTaxs.filter(ingTax => ingTax.id !== taxSelected.id)
                const sortedTaxs = [...taxsFiltered, taxSelected].sort((a: Tax, b: Tax) => a.minValue - b.minValue)
                setIngTaxs(sortedTaxs)
            }
            
            if(taxSelected.type === 'seguridad social'){
                const taxsFiltered = segTaxs.filter(segTax => segTax.id !== taxSelected.id)
                const sortedTaxs = [...taxsFiltered, taxSelected].sort((a: Tax, b: Tax) => a.minValue - b.minValue)
    ...
    const customTheme: CustomFlowbiteTheme['table'] = {
        "root": {
            ...
            "shadow": "absolute left-0 top-0 -z-10 w-full rounded-lg bg-white drop-shadow-md dark:bg-black",
        ...
        <>
            <div className="overflow-x-auto">
                <div className="flex mt-2 mb-10 p-3 rounded-md bg-gray-50 dark:bg-gray-800 items-center justify-between">
                    ...
                    <Button color="success" size='xs' onClick={() => setOpenModalAdd(true)}>Agregar impuesto</Button>
                </div>
                <div className="flex gap-8 w-full justify-between">
                    <Table theme={customTheme}>
                        <Table.Head>
                            <Table.HeadCell colSpan={3} className="text-center">Impuestos sobre seguridad social</Table.HeadCell>
                        </Table.Head>
                        <Table.Head>
                        ...
                        <Table.Body className="divide-y">
                            {
                                (segTaxs.length > 0) ? (segTaxs.map((segTax, i) => { 
                                    return <Table.Row key={i} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                        ...
                                        <Table.Cell className="flex justify-center gap-4">
                                            <button type="button" onClick={() => handleEdit(segTax)} className="font-medium text-yellow-300 dark:text-yellow-400">
                                                Editar
                                            </button>
                                            <button type="button" onClick={() => handleDelete(segTax)} className="font-medium text-red-600 dark:text-cyan-500">
                                                Eliminar
                                            </button>
                                        </Table.Cell>
                                    </Table.Row>
                                    ...
                                    <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                        <Table.Cell className="text-center bg-gray-50" colSpan={3}>No hay impuestos disponibles</Table.Cell>
                                    </Table.Row>
                                )
                            }
                        </Table.Body>
                    ...
                    <Table theme={customTheme}>
                        <Table.Head>
                            <Table.HeadCell colSpan={3} className="text-center">Impuestos sobre ingresos personales</Table.HeadCell>
                        </Table.Head>
                        <Table.Head>
                            <Table.HeadCell className="text-center">Rango</Table.HeadCell>
                            <Table.HeadCell className="text-center">Porciento</Table.HeadCell>
                        ...
                        <Table.Body className="divide-y">
                            {
                                (ingTaxs.length > 0) ? (ingTaxs.map((ingTax, i) => { 
                                    return <Table.Row key={i} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                        <Table.Cell className="text-center font-medium">
                                        ...
                                        <Table.Cell>
                                            <div className="flex justify-center gap-4">
                                                <button type="button" onClick={() => handleEdit(ingTax)} className="font-medium text-yellow-300 dark:text-yellow-400">
                                                    Editar
                                                </button>
                                                <button type="button" onClick={() => handleDelete(ingTax)} className="font-medium text-red-600 dark:text-cyan-500">
                                                    Eliminar
                                                </button>
                                            </div>
                                        </Table.Cell>
            ...
            <Modal show={openModalAdd} size="md" onClose={closeModalAdd} popup>
                <Modal.Header className="px-2 mb-5 flex items-center">
                    <h3 className="ml-5 text-xl font-medium text-gray-900 dark:text-white">Agregar nuevo impuesto</h3>
                </Modal.Header>
                <Modal.Body>
                    <div className="space-y-6">
                        <div>
                            ...
                            <Select id="type" name="type" required onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setNewTax({ ...newTax, [e.target.name]: e.target.value })}>
                                <option value="seguridad social" defaultChecked>Seguridad social</option>
                        ...
                        <div className="flex gap-2">
                            <div>
                                ...
                                <TextInput
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewTax({ ...newTax, [e.target.name]: e.target.value })}
                                    color={(newTax.minValue <= -1 && error.minValue === '') ? 'warning' : (error.minValue !== '') ? 'failure' : 'gray'}
                                    helperText={
                                        error.minValue !== '' && (
                                            <>
                            ...
                            <div>
                                ...
                                <TextInput
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewTax({ ...newTax, [e.target.name]: e.target.value })}
                                    color={(newTax.maxValue <= -1 && error.maxValue === '') ? 'warning' : (error.maxValue !== '') ? 'failure' : 'gray'}
                                    helperText={
                                        error.maxValue !== '' && (
                                            <>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewTax({ ...newTax, [e.target.name]: e.target.value })}
                                color={(newTax.percentage <= -1 && error.percentage === '') ? 'warning' : (error.percentage !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.percentage !== '' && (
                                        <>
                        ...
                        <div className="w-full">
                            <Button className="px-4" onClick={createNewTax} color="success">Agregar</Button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
            ...
            <Modal show={openModalEdit} size="md" onClose={closeModalEdit} popup>
                <Modal.Header className="px-2 mb-5 flex items-center">
                    <h3 className="ml-5 text-xl font-medium text-gray-900 dark:text-white">Editar impuesto { taxSelected && `${taxSelected.minValue}-${taxSelected.maxValue}` }</h3>
                </Modal.Header>
                <Modal.Body>
                    <div className="space-y-6">
                        <div>
                            ...
                            <Select id="type" name="type" required onChange={(e: React.ChangeEvent<HTMLSelectElement>) => taxSelected && setTaxSelected({...taxSelected, [e.target.name]: e.target.value})}>
                                <option value="seguridad social" defaultChecked={taxSelected && taxSelected.type === 'seguridad social'}>Seguridad social</option>
                                <option value="ingresos personales" defaultChecked={taxSelected && taxSelected.type === 'ingresos personales'}>Ingresos personales</option>
                            </Select>
                        </div>
                        <div className="flex gap-2">
                            <div>
                                ...
                                <TextInput
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => taxSelected && setTaxSelected({ ...taxSelected, [e.target.name]: e.target.value })}
                                    color={(taxSelected && taxSelected.minValue <= -1 && error.minValue === '') ? 'warning' : (error.minValue !== '') ? 'failure' : 'gray'}
                                    helperText={
                                        error.minValue !== '' && (
                                            <>
                                                <span className="font-medium">{error.minValue}</span>
                            ...
                            <div>
                                ...
                                <TextInput
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => taxSelected && setTaxSelected({ ...taxSelected, [e.target.name]: e.target.value })}
                                    color={(taxSelected && taxSelected.maxValue <= -1 && error.maxValue === '') ? 'warning' : (error.maxValue !== '') ? 'failure' : 'gray'}
                                    helperText={
                                        error.maxValue !== '' && (
                                            <>
                                                <span className="font-medium">{error.maxValue}</span>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => taxSelected && setTaxSelected({ ...taxSelected, [e.target.name]: e.target.value })}
                                color={(taxSelected && taxSelected.percentage <= -1 && error.percentage === '') ? 'warning' : (error.percentage !== '') ? 'failure' : 'gray'}
                                helperText={
                                    error.percentage !== '' && (
                                        <>
                                            <span className="font-medium">{error.percentage}</span>
                        ...
                        <div className="w-full">
                            <Button className="px-4" color="warning" onClick={editTax}>Editar</Button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
            ...
            <Modal show={openModalDelete} size="md" onClose={closeModalDelete} popup>
                ...
                <div className="text-center">
                    ...
                    <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        ¿Estás seguro de eliminar el impuesto {taxSelected && `${taxSelected.minValue} - ${taxSelected.maxValue}`}?
                    </h3>
                    <div className="flex justify-center gap-4">
